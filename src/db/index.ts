// ─────────────────────────────────────────────────────────────────────────────
// LaundryMate POS — Database (expo-sqlite v14)
// ─────────────────────────────────────────────────────────────────────────────
...
import type {
  Service, Addon, Customer, Order, OrderItem,
  Expense, ShopSettings, OrderAddon,
} from '../types';
...
export async function getDb(): Promise<SQLite.SQLiteDatabase> {
  if (!_db) {
    _db = await SQLite.openDatabaseAsync('laundrymate.db');
  }
  return _db;
}
...
async function isSeedNeeded(db: SQLite.SQLiteDatabase): Promise<boolean> {
  const row = await db.getFirstAsync<{ count: number }>(
    `SELECT COUNT(*) as count FROM ${TABLES.services}`
  );
...
async function runSeed(db: SQLite.SQLiteDatabase): Promise<void> {
  console.log('[DB] Running seed data...');

  // Services
  for (const s of SEED_SERVICES) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.services}
       (id, name, category, unit, price, duration, description, is_active, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [s.id, s.name, s.category, s.unit, s.price, s.duration, s.description, s.isActive ? 1 : 0, s.createdAt]
    );
  }
  ...
  for (const a of SEED_ADDONS) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.addons}
       (id, name, price, description, is_active)
       VALUES (?, ?, ?, ?, ?)`,
      [a.id, a.name, a.price, a.description, a.isActive ? 1 : 0]
    );
  }
  ...
  for (const c of SEED_CUSTOMERS) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.customers}
       (id, name, phone, email, address, loyalty_points, total_orders, total_spent, join_date, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [c.id, c.name, c.phone, c.email, c.address, c.loyaltyPoints, c.totalOrders, c.totalSpent, c.joinDate, c.createdAt, c.updatedAt]
    );
  }
  ...
  for (const o of SEED_ORDERS) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.orders}
      ...
      [
        ...
        o.servicesSubtotal, o.addonsSubtotal, o.deliveryFee, o.discount,
        o.total, o.amountPaid, o.change, o.paymentMethod, o.status,
        o.isPickup ? 1 : 0, o.pickupAddress, o.deliveryAddress, o.scheduledAt,
        o.driverName, o.deliveryStatus, o.notes,
        o.createdAt, o.updatedAt, o.readyAt, o.claimedAt,
      ]
    );

    for (const item of o.items) {
      await db.runAsync(
        `INSERT OR IGNORE INTO ${TABLES.order_items}
      ...
      for (const addon of item.addons) {
        await db.runAsync(
          `INSERT OR IGNORE INTO ${TABLES.order_addons}
           (order_item_id, order_id, addon_id, addon_name, price)
           VALUES (?, ?, ?, ?, ?)`,
          [item.id, o.id, addon.addonId, addon.addonName, addon.price]
        );
      }
    }
  }
  ...
  for (const e of SEED_EXPENSES) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.expenses}
       (id, category, category_label, description, amount, date, receipt, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [e.id, e.category, e.categoryLabel, e.description, e.amount, e.date, e.receipt, e.createdAt]
    );
  }

  // Settings
  const settingsEntries = Object.entries(SEED_SETTINGS);
  const now = new Date().toISOString();
  for (const [key, value] of settingsEntries) {
    await db.runAsync(
      `INSERT OR IGNORE INTO ${TABLES.settings} (key, value, updated_at) VALUES (?, ?, ?)`,
...
async function buildOrder(db: SQLite.SQLiteDatabase, row: any): Promise<Order> {
  ...
  const itemRows = await db.getAllAsync<any>(
    `SELECT * FROM ${TABLES.order_items} WHERE order_id = ? ORDER BY rowid`,
    [row.id]
  );
  ...
  const items: OrderItem[] = await Promise.all(
    itemRows.map(async (ir) => {
      const addonRows = await db.getAllAsync<any>(
        `SELECT * FROM ${TABLES.order_addons} WHERE order_item_id = ?`,
        [ir.id]
      );
      const addons: OrderAddon[] = addonRows.map((ar) => ({
        addonId: ar.addon_id,
        addonName: ar.addon_name,
        price: ar.price,
      }));
      return {
        id: ir.id,
        orderId: ir.order_id,
        serviceId: ir.service_id,
        serviceName: ir.service_name,
        qty: ir.qty,
        unit: ir.unit,
  ...
  return {
    ...
    items,
    servicesSubtotal: row.services_subtotal,
    addonsSubtotal: row.addons_subtotal,
...
export async function dbGetCustomers(): Promise<Customer[]> {
  const db = await getDb();
  const rows = await db.getAllAsync<any>(
    `SELECT * FROM ${TABLES.customers} ORDER BY name`
  );
...
export async function dbGetCustomerById(id: string): Promise<Customer | null> {
  ...
  const row = await db.getFirstAsync<any>(
    `SELECT * FROM ${TABLES.customers} WHERE id = ?`, [id]
  );
...
export async function dbUpdateCustomerStats(
  id: string,
  amountPaid: number
): Promise<void> {
  const db = await getDb();
...
export async function dbGetOrdersByCustomer(customerId: string): Promise<Order[]> {
  const db = await getDb();
  const rows = await db.getAllAsync<any>(
    `SELECT * FROM ${TABLES.orders} WHERE customer_id = ? ORDER BY created_at DESC`,
    [customerId]
  );
  return Promise.all(rows.map((r) => buildOrder(db, r)));
}
...
export async function dbInsertOrder(order: Order): Promise<Order> {
  ...
  for (const item of order.items) {
    ...
    for (const addon of item.addons) {
      await db.runAsync(
        `INSERT INTO ${TABLES.order_addons}
         (order_item_id, order_id, addon_id, addon_name, price)
         VALUES (?, ?, ?, ?, ?)`,
        [item.id, item.orderId, addon.addonId, addon.addonName, addon.price]
      );
    }
  }
...
export async function dbGetExpenses(): Promise<Expense[]> {
  const db = await getDb();
  const rows = await db.getAllAsync<any>(
    `SELECT * FROM ${TABLES.expenses} ORDER BY date DESC`
  );
...
export async function dbInsertExpense(
  ...
  await db.runAsync(
    `INSERT INTO ${TABLES.expenses}
     (id, category, category_label, description, amount, date, receipt, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
    [expense.id, expense.category, expense.categoryLabel, expense.description, expense.amount, expense.date, expense.receipt ?? null, now]
  );
...
export async function dbGetDashboardStats() {
  ...
  const todayRow = await db.getFirstAsync<any>(
    `SELECT
     ...
     FROM ${TABLES.orders}
     WHERE created_at >= ? AND amount_paid > 0`,
    [todayISO]
  );
  ...
  const processingRow = await db.getFirstAsync<any>(
    `SELECT COUNT(*) as cnt FROM ${TABLES.orders} WHERE status = 'processing'`
  );
  ...
  const pickupRow = await db.getFirstAsync<any>(
    `SELECT COUNT(*) as cnt FROM ${TABLES.orders} WHERE is_pickup = 1 AND status IN ('pending','processing','ready')`
  );

  const weekStart = new Date(); weekStart.setDate(weekStart.getDate() - 7); weekStart.setHours(0,0,0,0);
  const weekRow = await db.getFirstAsync<any>(
    `SELECT COALESCE(SUM(amount_paid), 0) as revenue FROM ${TABLES.orders} WHERE created_at >= ? AND amount_paid > 0`,
  ...
  return {
    ...
    processingCount:   processingRow?.cnt ?? 0,
